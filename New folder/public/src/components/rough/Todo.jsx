...
const FieldSelect = () => {
    ...
    const generateCsvData = () => {
        ...
        for (let i = 0; i < countCard; i++) {
            ...
            field.personal.forEach((field) => {
                ...
                const fieldValue = personalDataArray[i]?.personal[field.id]?.value || '';
                        ...
                        <div id={i + 1} className=' flex justify-center border lg:w-1/2 w-full mx-auto space-y-10 rounded-lg my-4 shadow-lg'>
                            ...
                            <div key={i} className='border-t w-full p-4 md:flex-row flex-col '>
                                                                    ...
                                                                    i === 0 ? (
                                                                        ...
                                                                        <div className=' flex flex-col w-full flex-wrap px-2 py-4 space-y-1'>
                                                                            ...
                                                                            <input id={input} type={field.company.find((cb) => cb.id === input)?.type} name=""
                                                                                value={importedData[i]?.[field.company.find((cb) => cb.id === input)?.name] || importedData[1]?.[field.company.find((cb) => cb.id === input)?.name] || undefined}
                                                                                className='border h-10 rounded-md px-2 py-2 outline-none'
                                                                                onChange={(e) => {
                                                                                    const newValue = {
                                ...
                                <div className='flex w-full justify-center'>
                                    <button type="reset" className='px-4 py-1 rounded-md bg-red-400 text-white'>reset</button>
                                </div>
                            </div >
                        </div >
                    </div>
    ...
    return (
        <>
            ...
            {showSelected ? (
                <div className='border top-0 flex justify-center fixed w-full z-50 bg-white shadow-md py-4 gap-x-8'>
                    <input
                        type="file"
                        accept=".csv"
                    ...
                    />
                    <button
                        onClick={() => fileInputRef.current.click()} // Trigger the file input click when the button is clicked
                        className="rounded-md bg-blue-400 text-white px-4 py-2"
                    >
                        Import CSV File
                    </button>
                    <button className='rounded-md bg-blue-400 text-white px-4 py-2' onClick={handleExportClick}>Export CSV</button>
